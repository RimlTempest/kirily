...
書き出しは PNG / WebP / JPEG。**JPEG を選ぶとプレビューの背景もその色になります**
— アルファが無い形式なので、市松模様のまま色を選ばせると、見えないまま選ぶことに
なるからです（[ADR-0018](docs/adr/0018-export-formats.md)）。

バケツは **AI の輪郭で止まり**、縁は画像から決まります。色だけでは白い襟と
白い背景を分けられないので、マスクがあるときはそれに聞きます
（[ADR-0019](docs/adr/0019-ai-bucket.md)）。マスク自体を直したいときは切れます。

縁は**あとから締められます**。モデルの解像度を引き伸ばした分だけマスクは
被写体より少し広く、そこは色では直せないアルファの問題だからです
（[ADR-0021](docs/adr/0021-edge-controls.md)）。1px 締めると残る縁が 84% 消えます。
「余白を詰める」で、見えているものの最小矩形まで一度に切り取れます。

切り抜きの**背後には色でも画像でも敷けます**。どの形式でも選べて、
枠取りは画像全体に対する `cover`（トリミングを動かしても背景は滑りません）。
スクリーンショットは **Cmd+V で直接開け**、結果は **クリップボードにコピー**
できます（[ADR-0022](docs/adr/0022-backdrop-and-clipboard.md)）。

動かせるものは 3 つあります — **表示**（手のひら・2 本指ドラッグ）、**背景**、
**被写体**。後ろの 2 つは同じ `Placement` で、プレビューと書き出しが同じ式を使います
（[ADR-0023](docs/adr/0023-moving-things.md)）。

ブラシは**ポインタにリングが付いて**、そのひと塗りがどこまで届くか塗る前に分かります。
太さは画像の画素で持っているので、画面上の大きさはズームで変わります
（[ADR-0026](docs/adr/0026-brush-ring.md)）。

表示は **90° ずつ回せます**。横長の写真は縦長の画面では幅で収まるので、
Pixel 7 では 1600×600 が 24% にしかならず、髪の隙間を指で塗れません。
回すと高さで収まって 35% になります。**書き出しの向きは変わりません**
