...
モデルは「被写体の内部が背景に似ている」場所で確信度を落とす。
**背景がほぼ白、肌もほぼ白**というイラストで顕著に出て、
シルエットは正しいのに顔が半透明に抜ける。

閾値で潰すと、守りたいアンチエイリアスの髪の輪郭まで固くなる。
代わりに `solidifyInterior()`（`packages/image-core/src/mask.ts`）が:

1. 画像の縁から、確信度の低い画素を辿って背景を塗る
2. そこからの距離を測る
3. `edgeBand`（既定 6px）までは**モデルの出力をそのまま残す**。
   その先 `ramp`（既定 8px）かけて不透明へ寄せる

ランプを入れないと、閉じた領域がシルエット付近に届いたときに
はっきりした継ぎ目が出る。

前景寄りの画素しか上げないので、本物の穴（取っ手の内側、腕の隙間）は
透明のまま残る。モデルはそこでは「迷い」ではなく 0 に近い値を返すため。

U²-Netp では無効にしている。320² では内部の細部が見えておらず、
閉じるべきものが無い一方、ぼかしで埋まった隙間を塗る危険だけが残る。

処理はモデルの解像度（1024²）で行う。安く済み、`edgeBand` が
ユーザーの画像サイズに依らずモデルが見た画素で測られる。

TypeScript のみで実装している（Rust の二重実装は無い）。AI worker は
WASM エンジンを読まないため。計測で問題になったら `kirily-mask` へ移す。
...
- デプロイ対象の静的アセットが 198 MiB 増える
- 同じ品質のモデルを 2 本持つぶん、検証対象が増える

## 実測（イラスト 1254×1254、背景も肌もほぼ白）

|          | U²-Netp (CPU)                           | IS-Net (WebGPU)           |
| -------- | --------------------------------------- | ------------------------- |
| 所要     | 2.2s                                    | 6.4s                      |
| 輪郭     | 320² を拡大したぶん、全周に数 px の甘さ | くっきり                  |
| アホ毛   | 出る（甘い）                            | 出る                      |
| 襟・肩   | **落ちる**（alpha 16）                  | 残る（alpha 255）         |
| 顔の内部 | 問題なし                                | `solidifyInterior` で解消 |

Python + onnxruntime の参照実装（rembg と同じ前処理・後処理）と
マスクを突き合わせ、前処理・後処理が忠実であることを確認済み。
...
- ~~IS-Net の fp16 版があれば 170 MiB → 85 MiB になる~~ → ADR-0014 で 84 MiB に差し替え済み
