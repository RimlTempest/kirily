...
`cloudflare/wrangler-action` の SHA を**推測で書いた。** `# v3` と添えて、
それらしい 40 桁を並べた。CI の `guard` は「SHA で固定されているか」しか見ないので、
**通ってしまう。**

ADR-0009 が「タグは可動だから commit SHA で固定する」と書いているのは、
まさにこういう取り違えを防ぐためである。固定されていることと、
**正しいものに固定されていること**は別である。

実際に引いたら v4.0.0 の `ebbaa158…` だった。直した。
