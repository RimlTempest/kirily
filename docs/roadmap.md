...
- [x] ブラシの届く範囲をポインタに表示（ADR-0026）
- [x] 表示の回転（横長の写真が縦長の画面で 24% → 35%。書き出しは変わらない、ADR-0027）
...
- [x] Actions のピンが実在し、コメントどおりであることを検査（ADR-0025）
- [x] 各 job が自分の使う道具を入れていることを検査（deploy が exit 127 で落ちた、ADR-0028）
