...
書く場所ごとに答える問いが違う。**同じことを 2 か所に書かない。**

| 書く場所       | 答える問い   | 例                                               |
| -------------- | ------------ | ------------------------------------------------ |
| コード         | **How**      | `solidifyInterior()` が縁から背景を辿る手順      |
| テストコード   | **What**     | 「本物の穴は透明のまま残る」                     |
| コミットログ   | **Why**      | 「背景と肌の明度が近く、内部が半透明に抜けたため」 |
| コードコメント | **Why not**  | 「閾値で潰さないのは髪の輪郭まで固くなるから」   |

### コードは How

処理の手順はコードそのもの。読んで分かるように書く。
分かりにくいなら、コメントで補うのではなく**コードを直す**。

### テストは What

テスト名は**振る舞いの主張**にする。実装の手順を書かない。
...
**何を変えたかは diff が語る。なぜ変えたかは diff から読めない。**
きっかけになった観測、測った数字、採らなかった選択肢を書く。

```
fix(ai): close low-confidence holes inside the subject

背景も肌もほぼ白いイラストで、シルエットは正しいのに顔の内部が
半透明に抜けた。参照実装と突き合わせ、モデル自身の挙動だと確認した。
```

### コメントは Why not

**コードを読めば分かることは書かない。** 書く価値があるのは、
読んだ人が「なぜこうしないのか」と思う場所。

- 採らなかった選択肢とその理由
  （なぜ crop は WASM を経由しないのか → バッファを 2 回コピーすることになる）
- 数値の根拠（なぜ 1600px なのか、なぜ 50MP なのか）
- 一見おかしい書き方が正しい理由
  （なぜ正方形へ引き伸ばすのか → モデルがそう学習されている）
- ブラウザ固有の落とし穴（Safari の `convertToBlob` の挙動）
- 仕様書への参照（`kirily-design.md §17`、`ADR-0008`）
