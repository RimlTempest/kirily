...
キャンバスは画面に見えている分だけを、ビューポート経由で元画像から
サンプリングして合成する（`renderViewport`）。プレビュー全体を合成して
CSS で拡大すると、髪を 20 画素だけ見ているときも画像全体ぶんの仕事をし、
しかも拡大したのは「プレビューを引き伸ばしたもの」になる。

副産物として、編集のたびにプレビュー解像度のマスクを作り直す必要がなくなる。

**高さは `h-full` で取らない。** 親が flex-grow と min-height で
サイズを決めていると、パーセント指定には解決先が無い。
実際にスマートフォンでキャンバスが 0x0 になった。`absolute inset-0` を使う。
