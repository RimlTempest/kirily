...
推論は**元画像**から行う。プレビューを経由すると、モデルが見る前に
リサンプルが 2 回余分に入って detail が落ちる。

入力は**正方形へ引き伸ばす**。比を保ちたくなるが、モデルは正方形に
リサイズした画像で学習されており、パディングすると被写体の画素が減る。
測定済み（ADR-0008）。
...
1024² のマスクを拡大しても、輪郭は「モデルの解像度でのモデルの輪郭」。
`refineMask()`（guided filter）が元画像をガイドに実エッジへ寄せる。
拡大は Lanczos。bilinear は対角を折れ線にするので階段が見える。

**精緻化は再セグメンテーションではない。** 素で当てると、
背景と同じ明るさの前景（白いハイライトなど）を背景と判断して消す。
届く範囲を輪郭補正に必要なぶんに限り、マスクが迷っている場所にしか
当てないこと。詳細は ADR-0008。

### 色の比較は OKLab で

RGB のユークリッド距離は明度差に鈍い。`pale-subject-on-white.png` では
背景と肌が RGB で 9.4 しか離れておらず、バケツの許容差を背景に合わせると
顔まで飲む。OKLab なら髪との分離比が 30x（RGB は 19x）。
`perceptualDistance()` を使う。

**許容差の既定値は測って決める。** この画像では背景自身のばらつきが
99% で 0.0070、髪の白いハイライトが 0.0093、肌が 0.0112 —
使える窓は 0.007〜0.009 しかない。feather を広く取ると背景が
半分だけ選択され、元の背景がうっすら残る。

### 縮小は支持幅を広げる

bilinear は縮小率に関わらず 2×2 しか読まない。4000px → 1024px で
画素の 4 分の 3 を捨ててエイリアスする。`resampleRgba` は比率で
フィルタの支持幅を広げるので、同じコードが面積平均になる。
